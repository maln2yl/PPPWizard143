...
*     see rtcm.c
*
* version : $Revision: 1.2 $ $Date: 2014-08-22 09:46:26 $
...
static const char rcsid[]="$Id: rtcm3.c,v 1.2 2014-08-22 09:46:26 privata Exp $";
...
/* decode type 1042: beidou ephemerides ------------*/
static int decode_type1042(rtcm_t *rtcm)
{
    ...
    if (i+499<=rtcm->len*8) {
        ...
        eph.omg   =getbits(rtcm->buff,i,32)*P2_31*SC2RAD; i+=32;
        eph.OMGd  =getbits(rtcm->buff,i,24)*P2_43*SC2RAD; i+=24;
        eph.tgd[0]=getbits(rtcm->buff,i,10)*0.1;          i+=10;
        eph.tgd[1]=getbits(rtcm->buff,i,10)*0.1;          i+=10;
    ...
    if (!strstr(rtcm->opt,"-EPHALL")) {
        //if (eph.iode==rtcm->nav.eph[sat-1].iode) return 0; /* unchanged */
	if (fabs(timediff(eph.toe,rtcm->nav.eph[sat-1].toe))<1E-3) return 0; /* unchanged */
    }
    rtcm->nav.eph[sat-1]=eph;
    rtcm->ephsat=sat;
    return 2;
...
/* decode ssr p message header ---------------------------------------------*/
static int decode_ssrp_head(rtcm_t *rtcm, int sys, int *sync, int *iod, int *dispersive, int *mw,
                            double *udint, int *hsize)
{
    ...
    *dispersive = getbitu(rtcm->buff,i, 1); i+= 1;
    *mw = getbitu(rtcm->buff,i, 1); i+= 1;
...
static int decode_ssr1(rtcm_t *rtcm, int sys)
{
    ...
    for (j=0;j<nsat&&i+121+np+ni+nj<=rtcm->len*8;j++) {
        prn     =(int)getbitu(rtcm->buff,i,np)+offp; i+=np;
	iode    =(int)getbitu(rtcm->buff,i,ni);      i+=ni;
        iodcrc  =(int)getbitu(rtcm->buff,i,nj);      i+=nj;
...
static int decode_ssr3(rtcm_t *rtcm, int sys)
{
    ...
    switch (sys) {
        case SYS_GPS: np=6; offp=  0; codes=codes_gps; ncode=19; break;
	...
	case SYS_CMP: np=6; offp=  0; codes=codes_bds; ncode=14; break;
        default: return sync?0:10;
    }
    for (j=0;j<nsat&&i+5+np<=rtcm->len*8;j++) {
        ...
        for (k=0;k<nbias&&i+19<rtcm->len*8;k++) {
            ...
            if (mode<=ncode) {
                cbias[codes[mode]-1]=(float)bias+1.0e-10;
            }
            else {
                {
		trace(2,"rtcm3 %d not supported mode: mode=%d\n",type,mode);
		fprintf(stderr, "rtcm3 %d not supported mode: mode=%d\n",type,mode);
		}
            }
        }
        if (!(sat=satno(sys,prn))) {
...
/* decode ssr p: satellite phase biases ---------------------------------------*/
static int decode_ssrp(rtcm_t *rtcm, int sys)
{
    const int codes_gps[]={
        CODE_L1C,CODE_L1P,CODE_L1W,CODE_L1Y,CODE_L1M,CODE_L2C,CODE_L2D,CODE_L2S,
        CODE_L2L,CODE_L2X,CODE_L2P,CODE_L2W,CODE_L2Y,CODE_L2M,CODE_L5I,CODE_L5Q,
    ...
    const int codes_bds[]={
        CODE_L2I,CODE_L2Q,CODE_L2X,CODE_L6I,CODE_L6Q,CODE_L6X,CODE_L7I,CODE_L7Q,
	CODE_L7X,CODE_L1D,CODE_L1P,CODE_L1X,CODE_L5D,CODE_L5P
    };
    const int *codes;
    double udint,bias,pbias[MAXCODE];
    int i,j,k,type,mode,sync,iod,nsat,prn,sat,nbias,np,offp,ncode;
    int dispersive=0, mw=0, discont, disc[MAXCODE];
    int integer,wli,n1[MAXCODE], nw[MAXCODE]; 
    
    type=getbitu(rtcm->buff,24,12);
    
    if ((nsat=decode_ssrp_head(rtcm,sys,&sync,&iod,&dispersive,&mw,&udint,&i))<0) {
        trace(2,"rtcm3 %d length error: len=%d\n",type,rtcm->len);
        return -1;
    }
    switch (sys) {
    ...
    for (j=0;j<nsat&&i+5+9+8+np<=rtcm->len*8;j++) {
        prn  =(int)getbitu(rtcm->buff,i,np)+offp; i+=np;
        nbias=getbitu(rtcm->buff,i, 5);      i+= 5;
	double yaw = getbitu(rtcm->buff,i, 9)/256.0;      i+= 9;
	double yawr = getbits(rtcm->buff,i, 8)/8192.0;    i+= 8;
//	fprintf(stderr, "PHASE %d %d %lf %lf ", prn, nbias, yaw, yawr);
 	for (k=0;k<MAXCODE;k++) {
		  pbias[k]=0.0;
		  disc[k]=0;
	}
        for (k=0;k<nbias&&i+5+1+2+4+20<rtcm->len*8;k++) {
            mode=getbitu(rtcm->buff,i, 5);         i+= 5;
	    integer=getbitu(rtcm->buff,i, 1);  i+= 1;
	    wli=getbitu(rtcm->buff,i, 2);      i+= 2;
            discont=getbitu(rtcm->buff,i, 4);     i+= 4;
            bias=getbits(rtcm->buff,i,20)/10000.0; i+=20;
            if (mode<=ncode) {
                pbias[codes[mode]-1]=(float)bias+1.0e-10;
                disc[codes[mode]-1]=discont;
		n1[codes[mode]-1]=integer;
		nw[codes[mode]-1]=wli;
            }
            else {
                {
...
//		fprintf(stderr, "\n");
        ...
        rtcm->ssr[sat-1].udi[4]=udint;
        rtcm->ssr[sat-1].iod[4]=iod;
	rtcm->ssr[sat-1].yawAngle = yaw;
	rtcm->ssr[sat-1].yawRate = yawr;
        
        for (k=0;k<MAXCODE;k++) {
            rtcm->ssr[sat-1].pbias[k]=(float)pbias[k];
            rtcm->ssr[sat-1].discontinuity[k]=disc[k];
	    rtcm->ssr[sat-1].n1i[k]=n1[k];
            rtcm->ssr[sat-1].wli[k]=nw[k];
        }
        rtcm->ssr[sat-1].update=1;
    }
    rtcm->disp=dispersive;
    rtcm->mw=mw;
    return sync?0:10;
}
...
static int decode_ssr4(rtcm_t *rtcm, int sys)
{
    ...
    switch (sys) {
        ...
        case SYS_QZS: np=4; ni= 8; nj= 0; offp=192; break;
       /* case SYS_CMP: np=6; ni=10; nj=24; offp=  0; break; */
        case SYS_CMP: np=6; ni=10; nj= 8; offp=  0; break; /* PATCH : IOD Beidou is now encoded on 8 bits */
        default: return sync?0:10;
    }
...
/* decode type 1264: VTEC iono -----------------------------------------------*/
static int decode_vtecSHF(rtcm_t *rtcm)
{
    int i=24+12,j,k,l=0,m=0,type,udi,sync,iod,provid,solid,qind,nlayers;
    double tow,udint;

    int height, nDeg,nOrd,nCoeffs,nSinCoeffs,nCosCoeffs;
    int c, s;
	
    type=getbitu(rtcm->buff,24,12);   
    trace(3,"rtcm3 %d length : len=%d\n",type,rtcm->len);

    /* Process header part of the SSR VTEC Ionosphere Spherical Harmonics 1264 */
    tow    =getbitu(rtcm->buff,i,20);   i+=20;
    adjweek(rtcm,tow);
    udi    =getbitu(rtcm->buff,i,4);    i+=4;		/* SSR Update Interval */
    sync   =getbitu(rtcm->buff,i,1);    i+=1;		/* Multiple Message Indicator */
    iod    =getbitu(rtcm->buff,i,4);    i+=4; 		/* IOD SSR */
    provid =getbitu(rtcm->buff,i,16);   i+=16;		/* SSR Provider ID */
    solid  =getbitu(rtcm->buff,i,4);    i+=4;		/* SSR Solution ID */
    qind   =getbitu(rtcm->buff,i,9);    i+=9;		/* VTEC Quality Indicator */
    nlayers=getbitu(rtcm->buff,i,2)+1;  i+=2;		/* Number of Ionospheric Layers */
    udint  =ssrudint[udi];				/* SSR Update Interval (s) */

    trace(3,"1264 header: type=%d,time=%s,udint=%d,sync=%d,",type,time_str(rtcm->time,2),udint,sync);
    trace(3,"iod=%d,provid=%d,solid=%d,qind=%d,nlayers=%d\n",iod,provid,solid,qind,nlayers);
	
    /* for each VTEC layer */
    for (j=0;j<nlayers;j++) {      
        /* Header part for an Ionospheric Layer */
        height            =getbitu(rtcm->buff,i,8)*10;      i+=8;
        nDeg              =getbitu(rtcm->buff,i,4)+1;       i+=4;
        nOrd              =getbitu(rtcm->buff,i,4)+1;       i+=4;
        rtcm->vtec.height =(double)height*1000.0;
        rtcm->vtec.nDeg   =nDeg;
        rtcm->vtec.nOrd   =nOrd;
        
        /* Calculate the number of spherical harmonic coefficients */
        nCoeffs   = (nDeg+1)*(nDeg+1)-(nDeg-nOrd)*(nDeg-nOrd+1);/* total number of SHF coefficients */
        nCosCoeffs= (nCoeffs+(nOrd+1))/2;		        /* number of cosine SHF coefficients */
        nSinCoeffs= (nCoeffs-(nOrd+1))/2;			/* number of sin SHF coefficients */

        trace(3,"1264 layer header: height=%d,degree=%d,order=%d,nCoeffs=%d,nCosCoeffs=%d,nSinCoeffs=%d \n",height,nDeg,nOrd,nCoeffs,nCosCoeffs,nSinCoeffs);
				
        if (i+(nCoeffs*16)==rtcm->len*8) {
            /* Read in Spherical Harmonic Cosine Coefficients */
            for (l=0;l<nCosCoeffs;l++) {
                c=getbits(rtcm->buff,i,16); /* cosine Coefficients */
		if (c == 32768) c=0;
                if (l < (MAXVTEC+1)*(MAXVTEC+2)/2) rtcm->vtec.cosCoeffs[l]=(double)c*0.005;
		i+=16;
            }
            /* Read in Spherical Harmonic Sine Coefficients */
            for (m=0;m<nSinCoeffs;m++) {
                s=getbits(rtcm->buff,i,16); /* Sine Coefficients */   
		if (s == -32768) s=0;
                if (m < (MAXVTEC+1)*(MAXVTEC+2)/2) rtcm->vtec.sinCoeffs[m]=(double)s*0.005;
                i+=16;
            }
	    }
    }
	
    trace(3,"#cosCoeffs read=%d, #sinCoeffs read=%d, finishing rtcm message index i=%d\n",l,m,i/8);
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
	...
	case 1258: ret=decode_ssr1(rtcm,SYS_CMP); break;
        ...
        case 1264: ret=decode_vtecSHF(rtcm); break;
	case 1265: ret=decode_ssrp(rtcm,SYS_GPS); break;
	case 1267: ret=decode_ssrp(rtcm,SYS_GAL); break;
	case 1270: ret=decode_ssrp(rtcm,SYS_CMP); break;
        case 1272: ret=decode_cnesstec(rtcm); break;
    }
    if (ret>=0) {
        type-=1000;
