...
static const char rcsid[]="$Id: rtcm3e.c,v 1.1 2014-08-20 12:56:44 privata Exp $";
                           ...
                           int solid)
{
    ...
    if (sys==SYS_GLO) {
        ...
        tow=time2gpst(rtcm->time,&week);
        epoch=ROUND(time2gpst(gpst2bdt(rtcm->time),NULL));
...
/* encode ssrp header ---------------------------------------------------------*/
static int encode_ssrp_head(int type, rtcm_t *rtcm, int sys, int nsat, int sync,
                           int iod, double udint, int refd, int provid, int solid)
{
    double tow;
    int i=24,np,msgno,epoch,week,udi;
    
    trace(4,"encode_ssrp_head: type=%d sys=%d nsat=%d sync=%d iod=%d udint=%.0f\n",
          type,sys,nsat,sync,iod,udint);
    
    switch (sys) {
        case SYS_GPS: msgno=1265+type; np=6; break;
//        case SYS_GLO: msgno=1266+type; np=6; break;
        case SYS_GAL: msgno=1267+type; np=6; break;
        case SYS_CMP: msgno=1270+type; np=6; break;
        default: return 0;
    }
    setbitu(rtcm->buff,i,12,msgno    ); i+=12; /* message number */
    ...
    setbitu(rtcm->buff,i,16,provid      ); i+=16; /* provider id */
    setbitu(rtcm->buff,i, 4,solid       ); i+= 4; /* solution id */
    setbitu(rtcm->buff,i, 1,rtcm->disp  ); i+= 1; /* dispersive */
    setbitu(rtcm->buff,i, 1,rtcm->mw    ); i+= 1; /* mw */
...
static int encode_ssr3(rtcm_t *rtcm, int sys, int sync)
{
    ...
    const int codes_qzs[]={
        CODE_L2C,CODE_L2S,CODE_L2L,CODE_L2S,CODE_L2L,CODE_L2X,CODE_L5I,CODE_L5Q,
        CODE_L5X
    };
    const int codes_bds[]={
    ...
    switch (sys) {
        case SYS_GPS: np=6; offp=  0; codes=codes_gps; ncode=sizeof(codes_gps)/sizeof(int); break;
        case SYS_GLO: np=5; offp=  0; codes=codes_glo; ncode=sizeof(codes_glo)/sizeof(int); break;
        case SYS_GAL: np=6; offp=  0; codes=codes_gal; ncode=sizeof(codes_gal)/sizeof(int); break;
        case SYS_CMP: np=6; offp=  0; codes=codes_bds; ncode=sizeof(codes_bds)/sizeof(int); break;
	case SYS_QZS: np=4; offp=192; codes=codes_qzs; ncode=sizeof(codes_qzs)/sizeof(int); break;
        default: return 0;
    }
    /* number of satellites */
    for (j=nsat=0;j<MAXSAT;j++) {
...
/* encode ssr p: satellite phase biases ---------------------------------------*/
static int encode_ssrp(rtcm_t *rtcm, int sys, int sync)
{
    const int codes_gps[]={
        CODE_L1C,CODE_L1P,CODE_L1W,CODE_L1Y,CODE_L1M,CODE_L2C,CODE_L2D,CODE_L2S,
        CODE_L2L,CODE_L2X,CODE_L2P,CODE_L2W,CODE_L2Y,CODE_L2M,CODE_L5I,CODE_L5Q,
    ...
    double udint=0.0;
    int i,j,k,iod=0,nsat,prn,nbias,np,offp,ncode;
    int code[MAXCODE],bias[MAXCODE],disc[MAXCODE],integ[MAXCODE],wli[MAXCODE];
    
    trace(3,"encode_ssrp: sys=%d sync=%d\n",sys,sync);
    
    switch (sys) {
        case SYS_GPS: np=6; offp=  0; codes=codes_gps; ncode=17; break;
        case SYS_GAL: np=6; offp=  0; codes=codes_gal; ncode=19; break;
	case SYS_CMP: np=6; offp=  0; codes=codes_bds; ncode=15; break;
        default: return 0;
    }
    /* number of satellites */
    for (j=nsat=0;j<MAXSAT;j++) {
    ...
    /* encode ssr header */
    i=encode_ssrp_head(0,rtcm,sys,nsat,sync,iod,udint,0,0,0);
    ...
    for (j=nsat=0;j<MAXSAT;j++) {
        ...
        for (k=nbias=0;k<ncode;k++) {
            ...
            code[nbias]=k;
            bias[nbias]=ROUND(rtcm->ssr[j].pbias[codes[k]-1]/0.0001);
            integ[nbias]=rtcm->ssr[j].n1i[codes[k]-1];
            wli[nbias]=rtcm->ssr[j].wli[codes[k]-1];
            disc[nbias++]=ROUND(rtcm->ssr[j].discontinuity[codes[k]-1]);
        }
	double yaw=ROUND(rtcm->ssr[j].yawAngle*256.0);
	double yawr=ROUND(rtcm->ssr[j].yawRate*8192.0);
        setbitu(rtcm->buff,i,np,prn-offp); i+=np; /* satellite id */
        setbitu(rtcm->buff,i, 5,nbias);    i+= 5; /* number of code biases */
        setbitu(rtcm->buff,i, 9,  yaw);    i+= 9; /* yaw */
        setbits(rtcm->buff,i, 8, yawr);    i+= 8; /* yaw rate */
        
        for (k=0;k<nbias;k++) {
            setbitu(rtcm->buff,i, 5, code[k]); i+= 5; /* signal indicator */
            setbitu(rtcm->buff,i, 1,integ[k]); i+= 1; /* integer */
            setbitu(rtcm->buff,i, 2,  wli[k]); i+= 2; /* wli */
            setbitu(rtcm->buff,i, 4, disc[k]); i+= 4; /* discontinuity */
            setbits(rtcm->buff,i,20, bias[k]); i+=20; /* code bias */
        }
    }
    rtcm->nbit=i;
    return 1;
...
/* encode vtec  */
static int encode_vtec(rtcm_t *rtcm, int sync)
{
    int i=24,week,type=1264,udi=3,iod=0,provid=0,solid=0,qind=0,nlayers=0;
    int nCoeffs,nSinCoeffs,nCosCoeffs,l,m;
    double tow;
    
    trace(3,"encode_vtec: sync=%d\n",sync);
    setbitu(rtcm->buff,i,12,type       ); i+=12; 	/* message no */
    tow=time2gpst(rtcm->time,&week);
    setbitu(rtcm->buff,i,20,tow);         i+=20;        /* gps epoch time */
    setbitu(rtcm->buff,i,4,udi);          i+=4;		/* SSR Update Interval */
    setbitu(rtcm->buff,i,1,sync);         i+=1;		/* Multiple Message Indicator */
    setbitu(rtcm->buff,i,4,iod);          i+=4; 	/* IOD SSR */
    setbitu(rtcm->buff,i,16,provid);      i+=16;	/* SSR Provider ID */
    setbitu(rtcm->buff,i,4,solid);        i+=4;		/* SSR Solution ID */
    setbitu(rtcm->buff,i,9,qind);         i+=9;		/* VTEC Quality Indicator */
    setbitu(rtcm->buff,i,2,nlayers);      i+=2;		/* Number of Ionospheric Layers */

    setbitu(rtcm->buff,i,8,ROUND(rtcm->vtec.height/10000.0));  i+=8;
    setbitu(rtcm->buff,i,4,rtcm->vtec.nDeg-1);	               i+=4;
    setbitu(rtcm->buff,i,4,rtcm->vtec.nOrd-1);	               i+=4;
    
    nCoeffs   = (rtcm->vtec.nDeg+1)*(rtcm->vtec.nDeg+1)-(rtcm->vtec.nDeg-rtcm->vtec.nOrd)*(rtcm->vtec.nDeg-rtcm->vtec.nOrd+1); /* total number of SHF coefficients */
    nCosCoeffs= (nCoeffs+(rtcm->vtec.nOrd+1))/2;			    /* number of cosine SHF coefficients */
    nSinCoeffs= (nCoeffs-(rtcm->vtec.nOrd+1))/2;			    /* number of sin SHF coefficients */

    /* Write in Spherical Harmonic Cosine Coefficients */
    for (l=0;l<nCosCoeffs;l++)
      {
      int c=ROUND(rtcm->vtec.cosCoeffs[l]/0.005);
      if (c <= -32768) c = -32768;
      if (c >=  32768) c = -32768;
      setbits(rtcm->buff,i,16,c);   i+=16;
      }
    /* Write in Spherical Harmonic Sine Coefficients */
    for (m=0;m<nSinCoeffs;m++)
      {
      int c=ROUND(rtcm->vtec.sinCoeffs[m]/0.005);
      if (c <= -32768) c = -32768;
      if (c >=  32768) c = -32768;
      setbits(rtcm->buff,i,16,c);   i+=16;
      }
...
extern int encode_rtcm3(rtcm_t *rtcm, int type, int sync)
{
    ...
    switch (type) {
        ...
        case 1264: ret=encode_vtec(rtcm,sync);         break;
        case 1265: ret=encode_ssrp(rtcm,SYS_GPS,sync); break;
	case 1267: ret=encode_ssrp(rtcm,SYS_GAL,sync); break;
	case 1270: ret=encode_ssrp(rtcm,SYS_CMP,sync); break;
        case 1272: ret=encode_cnesstec(rtcm,sync); break;
    }
    if (ret>0) {
        type-=1000;
